...
/*
#include "CoalaTaskCreatePoi.h"
#include "KismetProceduralMeshLibrary.h"

//Thread Worker Starts as NULL, prior to being instanced

FCoalaTaskCreatePoi* FCoalaTaskCreatePoi::Runnable = NULL;

FCoalaTaskCreatePoi::FCoalaTaskCreatePoi(ACoalaMeshActor* ret, FCoalaStreetRenderConfig usedConfig, UCoalaStreets* streets, FVector area_fixpoint, UProceduralMeshComponent* out)
{
	_out = out;
	...
	Thread = FRunnableThread::Create(this, TEXT("FCoalaTaskCreatePoi"), 0, TPri_BelowNormal);
}

FCoalaTaskCreatePoi::~FCoalaTaskCreatePoi()
{
	delete Thread;
	Thread = NULL;
}
...
//Run
uint32 FCoalaTaskCreatePoi::Run()
{
...
//stop
void FCoalaTaskCreatePoi::Stop()
{
	StopTaskCounter.Increment();
}

FCoalaTaskCreatePoi* FCoalaTaskCreatePoi::JoyInit(ACoalaMeshActor* ret, FCoalaStreetRenderConfig usedConfig, UCoalaStreets* streets, FVector area_fixpoint, UProceduralMeshComponent* out)
{
	//Create new instance of thread if it does not exist
	//		and the platform supports multi threading!
	if (!Runnable && FPlatformProcess::SupportsMultithreading())
	{
		Runnable = new FCoalaTaskCreatePoi(ret, usedConfig, streets, area_fixpoint, out);
	}
	return Runnable;
}

void FCoalaTaskCreatePoi::EnsureCompletion()
{
	Stop();
...
void FCoalaTaskCreatePoi::Shutdown()
{
	if (Runnable)
	{
...
ACoalaMeshActor*
FCoalaTaskCreatePoi::GetResult()
{
	return _ret;
}

bool FCoalaTaskCreatePoi::IsThreadFinished()
{
	if (Runnable) return Runnable->IsFinished();
	return false;
}
void FCoalaTaskCreatePoi::generateMesh(ACoalaMeshActor* ret, FCoalaStreetRenderConfig usedConfig, UCoalaStreets* streets, FVector area_fixpoint, UProceduralMeshComponent* out)
{
...
void
FCoalaTaskCreatePoi::generateStreetMesh(UProceduralMeshComponent*& out, TArray<TArray<FVector>> shapes, TArray<TArray<TArray<FVector>>> holes, TArray<int32> zOffset, int segmentIndexToCreateMesh, bool generateUVs, bool generateCollisions, bool stretchUVs, FCoalaStreetRenderConfig usedConfig)
{
